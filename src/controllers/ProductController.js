...
const getDetailsProductPublic = async (req, res) => {
  try {
    ...
    const response = await ProductService.getDetailsProductPublic(productId, userId);
    const { data, status, typeError, message, statusMessage } = response;
    return res.status(status).json({
      typeError,
      data,
...
const getDetailsProductPublicBySlug = async (req, res) => {
  try {
    ...
    const response = await ProductService.getDetailsProductPublicBySlug(slugId, userId);
    const { data, status, typeError, message, statusMessage } = response;
    return res.status(status).json({
      typeError,
      data,
...
const getAllProduct = async (req, res) => {
  try {
    ...
    const { data, status, typeError, message, statusMessage } = response;
    // console.log('chckkk data product', { data: data.products });
    return res.status(status).json({
      typeError,
      data,
      message,
    ...
    // console.log('Checkkk lỗi get all products', { e });
    return res.status(CONFIG_MESSAGE_ERRORS.INTERNAL_ERROR.status).json({
      message: 'Internal Server Error',
      data: null,
      status: 'Error',
